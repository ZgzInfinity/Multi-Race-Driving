 ...
 */
State Game::drawHudAnimationWorldTourPolePosition(Configuration& c, SoundPlayer& r){
    ...
    float levelPosition = -1.7f;

    float indexPosition = 5.7f;

    // Check the initial sound to reproduce
    // Check the initial sound to reproduce
    if (onMultiplayer){
        r.soundEffects[36]->stop();
        r.soundEffects[36]->play();
        while (r.soundEffects[36]->getStatus() != SoundSource::Stopped){}
    }
    else {
        switch(typeOfGame){
            case 0:
    ...
    for (int i = 0; i <= 350; i++){
        ...
        if (numberRacers > 0){
            ...
            positionInRace.setPosition(initial * 3.9f, down - levelPosition * float(positionInRace.getCharacterSize()));
            ordinalPos.setPosition(initial * indexPosition + positionInRace.getLocalBounds().width + 5.f,
        ...
        if (indexCheckPoint % 4 != 0){
            ...
            checkPoint.setPosition(initial * 3.49f, up - checkPointIndicator * float(checkPoint.getCharacterSize()));
        ...
        if (levelPosition < 0.95f){
            levelPosition += 0.011;
        }

        if (indexPosition > 3.9f){
            indexPosition -= 0.0065;
        }
 ...
 * Draw the initial animation of the HUD
 */
State Game::drawHudAnimationOutRunDrivingFuryDemarrage(Configuration& c, SoundPlayer& r){
    ...
    for (int i = 0; i <= 350; i++){
        ...
        if (typeOfGame == 1){
            ...
            if (typeOfGame == 3){
                ...
                checkPoint.setPosition(initial * 3.6f, up - checkPointIndicator * float(checkPoint.getCharacterSize()));
            }
            else {
                // Derramage
                if (distanceGoalCar > 0){
                    ...
                    checkPoint.setPosition(initial * 3.8f, up - checkPointIndicator * float(checkPoint.getCharacterSize()));
                }
                else {
                    // The player goes first and the goal car goes behind
                    checkPoint.setString("LEAD DISTANCE");
                    checkPoint.setPosition(initial * 4.0f, up - checkPointIndicator * float(checkPoint.getCharacterSize()));
                }
            }
            c.w.draw(checkPoint);
                                              ...
                                              mutex tourLandScapesMutex[])
{
    ...
    LandScape m(c, path + to_string(i + 1) + "/", "bg.png", timeLandscape, typeOfGame, numberRacers, onMultiplayer, typeOfGameMultiplayer);
...
{
    // Load the landscape using the correspond path
    LandScape m(c, path, "bg.png", timeLandscape, typeOfGame, numberRacers, false, 0);
...
State Game::loadWorldTourPolePositionConf(Configuration& c){
    ...
    else if (onMultiplayer && typeOfGameMultiplayer != 0){
 ...
 * @return
 */
State Game::playWorldTourPolePositionMultiplayer(Configuration &c, SoundPlayer& r) {

    if (onMultiplayer && !firstRace){
        posArrival = codePlayerInGroup;
    }
    ...
    while (!finalGame && !arrival && !cancelledRaceGuest && !cancelledRaceOwner && c.window.isOpen()){
        ...
        // Update the status of the game
        if (typeOfGameMultiplayer == 0){
            updateGameWorldTourStatusMultiplayer(c, r, action, direction, terrain);
        }
        else {
            updateGamePolePositionStatusMultiplayer(c, r, action, direction, terrain);
        }
    ...
    if (arrival) {
        ...
        // Wait until thread counter has finished

        // Wall to wait until all the players have finished the race
        RectangleShape shape;
        ...
        panel.setPosition((c.w.getSize().x / 2.f) - 250.0f * c.screenScale, c.w.getSize().y / 2.f - 240.0f * c.screenScale);
        panel.setSize(sf::Vector2f(494.0f * c.screenScale, 480.0f * c.screenScale));
        panel.setOutlineColor(c.colorBorderPanelMultiplayerMenu);
        panel.setOutlineThickness(5.0f * c.screenScale);
        panel.setTexture(&c.backgroundMultiplayerMembersGroupPanel, true);

        Text info;
        info.setString("RACE CLASSIFICATION");
        info.setFillColor(Color(10, 201, 235));
        info.setOutlineColor(Color(3, 39, 8));
        info.setOutlineThickness(3.0f * c.screenScale);
        info.setCharacterSize(static_cast<unsigned int>(int(45.0f * c.screenScale)));
        info.setStyle(Text::Bold);
        info.setFont(c.fontTitleMultiplayerMembersGroupMenu);
        info.setPosition(c.w.getSize().x / 2.f - info.getLocalBounds().width / 2.f, c.w.getSize().y / 2.f - 180.0f * c.screenScale);

        // Counter of players registered
        int numPlayers = 1, localPlayers = 1, numRacers = 1;

        // Control that all the players are registered
        bool finishedRegister = false, checkingRegister = false;
        mtx3.lock();
        int players = groupDataPlayers.size(), nplayers;
        multiplayerCarsRanking.reserve(players);
        mtx3.unlock();
        nplayers = players;
        ...
        Text playerName;
        playerName.setCharacterSize(static_cast<unsigned int>(int(35.0f * c.screenScale)));
        ...
        bool written = false, checkWritten = false;

        mtx3.lock();
        bool checkingGroup = cancelledRaceOwner;
        bool outOwner = cancelledRaceGuest;
        mtx3.unlock();



        // Control the vehicle selected by the other rivals
        controlRacerGoal = thread(storeRivalPlayers2, this, ref(numPlayers), ref(finishedRegister),
                                  ref(cancelledRaceOwner), ref(cancelledRaceGuest), ref(vehiclesRead), ref(written));

        // Reproduce soundtrack
        if (posArrival == 1){
            r.soundTracks[20]->play();
        }
        else if (posArrival == 2){
            r.soundTracks[21]->play();
        }
        else {
            r.soundTracks[22]->play();
        }
        ...
        while (!checkingRegister && !checkingGroup && !outOwner){

            if (c.window.pollEvent(e) && e.type == Event::Closed){
                Text complainText;
                ...
                for (int i = 0; i <= 80; i++){
                    ...
                    switch(typeOfVehicle){
                        case 0:
                            player.draw(c, r, Vehicle::Action::NONE, Vehicle::Direction::RIGHT,
                                        currentMap->getElevation(player.getPosY()), -1);
                            break;
                        case 1:
                            player2.draw(c, r, Vehicle::Action::NONE, Vehicle::Direction::RIGHT,
                                         currentMap->getElevation(player2.getPosY()), -1);
                            break;
                        case 2:
                            player3.draw(c, r, Vehicle::Action::NONE, Vehicle::Direction::RIGHT,
                                         currentMap->getElevation(player3.getPosY()), -1);
                            break;
                        case 3:
                            player4.draw(c, r, Vehicle::Action::NONE, Vehicle::Direction::RIGHT,
                                         currentMap->getElevation(player4.getPosY()), -1);
                            break;
                        case 4:
                            player5.draw(c, r, Vehicle::Action::NONE, Vehicle::Direction::RIGHT,
                                         currentMap->getElevation(player5.getPosY()), -1);
                            break;
                        case 5:
                            player6.draw(c, r, Vehicle::Action::NONE, Vehicle::Direction::RIGHT,
                                         currentMap->getElevation(player6.getPosY()), -1);
                    }
                ...
                checkingGroup = cancelledRaceOwner;
                outOwner = cancelledRaceGuest;
                mtx3.unlock();
                ...
                mtx3.lock();
                int numberPlayers = groupDataPlayers.size();
                for (int i = 1; i <= numberPlayers; i++){

                    if (vehiclesRead[i - 1] != 0){
                        playerName.setString(to_string(i) + ". " + multiplayerCarsRanking[i - 1].getNickNamePlayer());
                ...
                if (numPlayers != numRacers){
                    numRacers = numPlayers;
                    r.soundEffects[127]->stop();
                    r.soundEffects[127]->play();
                }
                mtx3.unlock();
        ...
        if (checkingGroup){
            if (modeMultiplayer == 0){
                ...
                controlRacerGoal.join();
                controlRaceOwner.join();

                // Stop the soundtrack
                if (posArrival == 1){
                    r.soundTracks[20]->stop();
                }
                else if (posArrival == 2){
                    r.soundTracks[21]->stop();
                }
                else {
                    r.soundTracks[22]->stop();
                }

                // The race done is cleaned
                firstRace = false;
                ...
                for (int i = 0; i <= 120; i++){
                    ...
                    if (c.window.pollEvent(e) && e.type == Event::Closed){
                        ...
                        multiplayerIndicator.setString("SORRY, ANY MEMBERS REGISTERED");
                ...
                // Return to the selection group menu
                return MULTIPLAYER_NAME_GROUP;
            }
        }
        else if (outOwner){
            ...
            for (int i = 0; i <= 120; i++){
                ...
                if (c.window.pollEvent(e) && e.type == Event::Closed){
                    ...
                    multiplayerIndicator.setString("THE OWNER CANCELED THE GROUP, WAIT");
        ...
        for (int j = 0; j <= 160; j++){
            ...
            multiplayerIndicator.setString("ALL OPONENTS CLASSIFIED");
            if (c.isDefaultScreen){
                multiplayerIndicator.setPosition(c.w.getSize().x / 2.f - multiplayerIndicator.getLocalBounds().width / 2.f,
                                                 c.w.getSize().y / 2.f + 260.0f * c.screenScale);
            }
    ...
    if (modeMultiplayer == 0){
        ...
        if (!cancelledRaceOwner){
            ...
            reasonDetection1 = "SORRY, ALL THE MEMBERS HAVE";
            reasonDetection2 = "LEFT THE GROUP, PLEASE WAIT";
...
State Game::showsInitialAnimation(Configuration &c, SoundPlayer& r) {
    ...
    // Draw the semaphore
    if ((onMultiplayer && codePlayerInGroup > 2) || typeOfGame == 0 || (typeOfGame == 2 && numberRacers >= 2)){
        Sprite s;
        s.setTexture(textures[4], true);
        s.setScale(2.5f * c.screenScale, 2.5f * c.screenScale);
        s.setPosition((float)c.w.getSize().x / 2.f - (s.getTexture()->getSize().x * s.getScale().x / 2.f),
                             c.w.getSize().y / 2.f - 180.0f * c.screenScale);
        c.w.draw(s);
    ...
    if ((!onMultiplayer && typeOfGame != 3) || onMultiplayer){
    ...
    else if (!onMultiplayer && typeOfGame == 3){
        Text indicatorStart;
    ...
    if (!finalGame && !arrival) {
        ...
        for (TrafficCar &v : cars) {
            if (v.getPosY() + DEL_DISTANCE < currentMap->getCameraPosY()) {
                v.update(lastY, lastY + float(c.renderLen) / DENSITY_SPACE, c.maxAggressiveness, c.level, typeOfGame);
 ...
 * @param c is the configuration of the game
 */
void Game::updateGameWorldTourStatusMultiplayer(Configuration &c, SoundPlayer& r, Vehicle::Action &action,
                                          Vehicle::Direction &direction, int& terrain)
{
...
void Game::updateGamePolePositionStatusMultiplayer(Configuration &c, SoundPlayer& r, Vehicle::Action &action,
                                                   Vehicle::Direction &direction, int& terrain)
{
    switch(typeOfVehicle){
        case 0:
    ...
    if (currentMap->isOutSideLandScape()) {
        ...
        if (level < 0){
            ...
            if (!currentMap->isFinalLandScape()){
                ...
                if (onMultiplayer && typeOfGameMultiplayer != 0){
                    if (lapsDone <= numberLaps){
                        if (currentMap->isMiddleMap()){
                            currentMap->addNewLandScape(&tourLandScapes[landScapeSelected]);
                            checkPointPositions = tourLandScapes[landScapeSelected].getCheckPointPositions();
...
void Game::storeRivalPlayers1(int& numPlayers, bool& finishedRegister, bool& cancelledGroup, vector<int>& vehiclesRead){
...
void Game::storeRivalPlayers2(int& numPlayers, bool& finishedRegister, bool& cancelledGroup, bool& cancelledOwner, vector<int>& vehiclesRead,
                              bool& written){

    // Local variables to store the information
    bool finished = false, cancelled = false, cancelled2 = false;
    int playersRegistered = 1, totalPlayers;

    // Store your position in the list of players
    mtx3.lock();
    int idPlayer = codePlayerInGroup;
    multiplayerCarsRanking[posArrival - 1] = MultiplayerData(idPlayer, nickNameMultiplayer);
    multiplayerCarsRanking[posArrival - 1].setVehicleType(typeOfVehicle);
    multiplayerCarsRanking[posArrival - 1].setColorVehicle(colorCarSelected);
    vehiclesRead[posArrival - 1] = 1;
    written = true;
    mtx3.unlock();
    ...
    totalPlayers = groupDataPlayers.size();
    cancelled = cancelledGroup;
    cancelled2 = cancelledOwner;
    mtx3.unlock();

    bool found;

    while (!cancelled && !cancelled2 && playersRegistered != totalPlayers){
        // Check if any of the players has sent his vehicle
        for (int i = 1; i <= totalPlayers; i++){
            if (i != codePlayerInGroup){
                mtx3.lock();
                ...
                mtx3.unlock();
                if (registered == 0){
                    Tuple t = Tuple("ARRIVED_GOAL", nickNameGroupMultiplayer, to_string(i), namePlayer, "?C");
                    Tuple r = winLindadriver.readNoteX(t, found);
                    ...
                    if (found && r.get(1) != NF){
                        ...
                        winLindadriver.removeNote(r);
                        numPlayers++;
                        int positionGoal = stoi(r.get(5));
                        multiplayerCarsRanking[positionGoal - 1] = MultiplayerData(i, namePlayer);

                        // Get the car and color of the vehicle of the player
                        int typeVehicle = groupDataPlayers[i - 1].getVehicle();
                        int colorVehicle = groupDataPlayers[i - 1].getColor();

                        multiplayerCarsRanking[positionGoal - 1].setVehicleType(typeVehicle);
                        multiplayerCarsRanking[positionGoal - 1].setColorVehicle(colorVehicle);
...
State Game::selectionVehicleMenu(Configuration& c, SoundPlayer& r){
    ...
    if (backSpacePressed){
        ...
        if (onMultiplayer){
            ...
            if (modeMultiplayer == 0){
                ...
                if (checkingGroup){
                    ...
                    Text complainText;
                    complainText.setString("NO GUESTS IN GROUP, WAIT");
                    complainText.setCharacterSize(static_cast<unsigned int>(int(50.0f * c.screenScale)));
                    complainText.setFont(c.fontTitleMultiplayerMenu);
                    complainText.setStyle(Text::Bold);
                    complainText.setFillColor(c.colorTitleTextMultiplayerMenu);
                ...
                Text complainText;
                complainText.setString("CANCELING VEHICLE SELECTION, WAIT");
                complainText.setCharacterSize(static_cast<unsigned int>(int(50.0f * c.screenScale)));
                complainText.setFont(c.fontTitleMultiplayerMenu);
                complainText.setStyle(Text::Bold);
                complainText.setFillColor(c.colorTitleTextMultiplayerMenu);
                ...
                if (cancelledVehicle){
                    ...
                    Text complainText;
                    complainText.setString("CANCELED VEHICLE SELECTION BY OWNER, WAIT");
                    complainText.setCharacterSize(static_cast<unsigned int>(int(40.0f * c.screenScale)));
                    complainText.setFont(c.fontTitleMultiplayerMenu);
                    complainText.setStyle(Text::Bold);
                    complainText.setFillColor(c.colorTitleTextMultiplayerMenu);
                    complainText.setOutlineColor(c.colorTitleBorderMultiplayerMenu);
                ...
                Text complainText;
                complainText.setString("CANCELING SUSCRIPTION, WAIT");
                complainText.setCharacterSize(static_cast<unsigned int>(int(50.0f * c.screenScale)));
                complainText.setFont(c.fontTitleMultiplayerMenu);
                complainText.setStyle(Text::Bold);
                complainText.setFillColor(c.colorTitleTextMultiplayerMenu);
        ...
        if (onMultiplayer){
            ...
            mtx3.lock();
            groupDataPlayers[codePlayerInGroup - 1].setVehicleType(typeOfVehicle);
            groupDataPlayers[codePlayerInGroup - 1].setColorVehicle(colorCarSelected);
            mtx3.unlock();
            ...
            // Control the vehicle selected by the other rivals
            vehicleRestPlayers = thread(storeRivalPlayers1, this, ref(numPlayers), ref(finishedRegister), ref(cancelledGroup), ref(vehiclesRead));
        ...
        switch(typeOfVehicle){
            case 0:
                ...
                player = Motorbike(speedVehicles[optionSelected][colorSelected], SPEED_FACTOR,
                         ...
                         COUNTER, "Data/Vehicles/Motorbikes/Motorbike" + to_string(colorSelected + 1), positX, positY,
            ...
            case 1:
                ...
                player2 = Devastator(speedVehicles[optionSelected][colorSelected], SPEED_FACTOR,
                          ...
                          COUNTER,  "Data/Vehicles/Devastators/Devastator" + to_string(colorSelected + 1), positX, positY,
                          vehicleNames[optionSelected][colorSelected], angleVehicles[optionSelected][colorSelected],
                          motorNames[optionSelected][colorSelected]);
                break;
            case 2:
                ...
                player3 = Minivan(speedVehicles[optionSelected][colorSelected], SPEED_FACTOR,
                          speedVehicles[optionSelected][colorSelected] * ACCELERATION_INCREMENT / MAX_SPEED, 4.f, 4.f,
                          COUNTER,  "Data/Vehicles/Minivans/Minivan" + to_string(colorSelected + 1), positX, positY,
                          vehicleNames[optionSelected][colorSelected], angleVehicles[optionSelected][colorSelected],
                          motorNames[optionSelected][colorSelected]);
                break;
            case 3:
                ...
                player4 = Truck(speedVehicles[optionSelected][colorSelected], SPEED_FACTOR,
                          ...
                          COUNTER, "Data/Vehicles/Trucks/Truck" + to_string(colorSelected + 1), 0.0f, positY,
                          vehicleNames[optionSelected][colorSelected], angleVehicles[optionSelected][colorSelected],
                          motorNames[optionSelected][colorSelected]);
                break;
            case 4:
                ...
                player5 = FormulaOne(speedVehicles[optionSelected][colorSelected], SPEED_FACTOR,
                          speedVehicles[optionSelected][colorSelected] * ACCELERATION_INCREMENT / MAX_SPEED, 4.f, 4.f,
                          COUNTER, "Data/Vehicles/FormulaOnes/FormulaOne" + to_string(colorSelected + 1), positX, positY,
                          vehicleNames[optionSelected][colorSelected], angleVehicles[optionSelected][colorSelected],
                          motorNames[optionSelected][colorSelected]);
                break;
            case 5:
                ...
                player6 = Police(max_speed, SPEED_FACTOR, speedVehicles[optionSelected][colorSelected] * ACCELERATION_INCREMENT / MAX_SPEED,
                                 4.f, 4.f, COUNTER, "Data/Vehicles/Police", positX, positY, vehicleNames[optionSelected][colorSelected],
                                 angleVehicles[optionSelected][colorSelected], motorNames[optionSelected][colorSelected]);
        }

        if (onMultiplayer){

            // Store all the vehicle multi player rivals
            storingMultiplayerCars();
...
void Game::controlGuestPulses(bool& circuitSelected, bool& cancelledGroup, bool& escape){
    ...
    while (!pathSelected && !cancelled && !escaped){
        ...
        for (int i = 2; i <= groupDataPlayers.size(); i++){
            ...
            // Sleep the thread in order to not consume many attempts
...
void Game::sendGuestPulses(bool& circuitSelected, bool& finishedGroup, bool &cancelledCicruit){
    ...
    while (!pathSelected && !fail && !cancelledCicruit){
        ...
        // Tuple that represents the pulse received
        t = Tuple("CIRCUIT_SELECTED", nickNameGroupMultiplayer, nickNameMultiplayer, "?X", "?Y");
    ...
    if (startPressed){
        ...
        if (onMultiplayer){
            ...
            for (int i = 2; i <= numberPlayersGroup; i++){
                ...
                // Inform that the group is cancelled
                Tuple t = Tuple("CIRCUIT_SELECTED", nickNameGroupMultiplayer, name, to_string(landScapeSelected), to_string(numberLaps));
                winLindadriver.postNote(t);
            ...
            Text complainText;
            complainText.setString("CIRCUIT SELECTED, WAIT");
            complainText.setCharacterSize(static_cast<unsigned int>(int(35.0f * c.screenScale)));
            complainText.setFont(c.fontTitleMultiplayerMenu);
            complainText.setStyle(Text::Bold);
            complainText.setFillColor(c.colorTitleTextMultiplayerMenu);
        ...
        if (onMultiplayer){
            ...
            if (numberPlayersGroup > 1){
                for (int i = 2; i <= numberPlayersGroup; i++){
                    ...
                    Text complainText;
                    complainText.setString("CIRCUIT SELECTION CANCELED, WAIT");
                    complainText.setCharacterSize(static_cast<unsigned int>(int(35.0f * c.screenScale)));
                    complainText.setFont(c.fontTitleMultiplayerMenu);
                    complainText.setStyle(Text::Bold);
                    complainText.setFillColor(c.colorTitleTextMultiplayerMenu);
...
State Game::introduceNameMultiplayer(Configuration& c, SoundPlayer& r){
    ...
    while (!startPressed && !escapePressed) {
        ...
        Text info3;
        info3.setString("PRESS BACKSPACE TO DELETE LETTERS");
        info3.setFillColor(Color(10, 201, 235));
        info3.setOutlineColor(Color(3, 39, 8));
        info3.setCharacterSize(static_cast<unsigned int>(int(25.0f * c.screenScale)));
        info3.setOutlineThickness(3.0f * c.screenScale);
        info3.setStyle(Text::Bold);
        info3.setFont(c.fontTitleMultiplayerNameMenu);
        info3.setPosition(c.w.getSize().x / 2.f - info3.getLocalBounds().width / 2.f, c.w.getSize().y / 2.f - 0.0f * c.screenScale);
        c.w.draw(info3);
            ...
            while (!startPressed && !escapePressed) {
                ...
                if (c.window.pollEvent(e)) {
                    ...
                    if (e.type == Event::Closed){
                        ...
                        // Control if the back space key has been pressed
                        if (c.window.hasFocus() && Keyboard::isKeyPressed(Keyboard::BackSpace)){
                            // Some letters have been introduced before
                            if (lettersIntroduced > 0){
                                // Remove the last letter and
                                if (lettersIntroduced == 8 ){
                                    // Remove last letter and insert _
                                    nickNameMultiplayer.pop_back();
                                    nickNameMultiplayer += "_";
                                }
                                else {
                                    // Remove the _ and the last letter
                                    nickNameMultiplayer.pop_back();
                                    nickNameMultiplayer.pop_back();
                                    nickNameMultiplayer += "_";
                                }
                                // Decrement the number of letters of the name
                                lettersIntroduced--;
                                r.soundEffects[116]->stop();
                                r.soundEffects[116]->play();
                            }
        ...
        while (!fail && !done){
             ...
             if (found && r.get(1) != NF){
                winLindadriver.removeNote(r);
                 ...
                 if (randomMultiplayerJoined){
                    t = Tuple("CANCELLED_SUSCRIPTION", "UNKNOWN", nickNameMultiplayer);
                 }
                 else {
                    t = Tuple("CANCELLED_SUSCRIPTION", nickNameGroupMultiplayer, nickNameMultiplayer);
                 }
                 r = winLindadriver.readNoteX(t, found);
                 if (found && r.get(1) != NF){
                    winLindadriver.removeNote(r);
...
State Game::creationOfGroups(Configuration& c, SoundPlayer& r){
    ...
    if (modeMultiplayer == 0){
        ...
        while (!startPressed && !escapePressed && !checkingFullGroup){
            ...
            if ((c.window.hasFocus() && Keyboard::isKeyPressed(Keyboard::Enter)) || checkingFullGroup) {
                ...
                if (numberPlayersGroup == 1){
                    ...
                    // Complaining text
                    Text complainText;
                    complainText.setString("SORRY, ANY MEMBERS REGISTERED");
                    complainText.setCharacterSize(static_cast<unsigned int>(int(45.0f * c.screenScale)));
                    complainText.setFont(c.fontTitleMultiplayerMenu);
                    complainText.setStyle(Text::Bold);
                    complainText.setFillColor(c.colorTitleTextMultiplayerMenu);
                ...
                Text complainText;
                if (checkingFullGroup){
                    complainText.setString("THE GROUP IS COMPLETED, WAIT");
                }
                else {
                    complainText.setString("THE GROUP IS CLOSED, WAIT");
                }
        ...
        if (escapePressed){
            ...
            // Complaining text
            Text complainText;
            complainText.setString("CANCELLING THE GROUP, WAIT");
            complainText.setCharacterSize(static_cast<unsigned int>(int(45.0f * c.screenScale)));
            complainText.setFont(c.fontTitleMultiplayerMenu);
            complainText.setStyle(Text::Bold);
            complainText.setFillColor(c.colorTitleTextMultiplayerMenu);
        ...
        if (!randomMultiplayerJoined){
            ...
            // Control if the player has joined
            bool partialSucess = false;
            ...
            // Throw a thread that controls the incorporation of new players
            capturerGroups = thread(capturerOfGroups, this, ref(success), ref(fail), ref(partialSucess));
            ...
            while (!escapePressed && !success){
                ...
                if (checkingFail){
                    ...
                    Text complainText;
                    complainText.setString("GROUP CANCELED, WAIT");
                    complainText.setCharacterSize(static_cast<unsigned int>(int(45.0f * c.screenScale)));
                    complainText.setFont(c.fontTitleMultiplayerMenu);
                    complainText.setStyle(Text::Bold);
                    complainText.setFillColor(c.colorTitleTextMultiplayerMenu);
                ...
                if (c.window.hasFocus() && Keyboard::isKeyPressed(Keyboard::Escape)) {
                    if (!partialSucess){
                        ...
                        // Inform that the group is cancelled
                        Tuple t = Tuple("CANCELLED_SUSCRIPTION", nickNameGroupMultiplayer, nickNameMultiplayer);

                        winLindadriver.postNote(t);

                        Text complainText;
                        complainText.setString("CANCELLING SUSCRIPTION, WAIT");
                        complainText.setCharacterSize(static_cast<unsigned int>(int(45.0f * c.screenScale)));
                        complainText.setFont(c.fontTitleMultiplayerMenu);
                        complainText.setStyle(Text::Bold);
                        complainText.setFillColor(c.colorTitleTextMultiplayerMenu);
                        ...
                        Text complainText;
                        complainText.setString("SORRY, YOUR PETITION HAS BEEN CONFIRMED");
                        complainText.setCharacterSize(static_cast<unsigned int>(int(45.0f * c.screenScale)));
                        complainText.setFont(c.fontTitleMultiplayerMenu);
                        complainText.setStyle(Text::Bold);
                        complainText.setFillColor(c.colorTitleTextMultiplayerMenu);
            ...
            nickNameGroupMultiplayer = "UNKNOWN";

            // Control if the join has ended bad
            bool fail = false, success = false, partialSucess = false;
            ...
            while (!escapePressed && !success && !fail){
                ...
                if (c.window.hasFocus() && Keyboard::isKeyPressed(Keyboard::Escape)){
                    if (!partialSucess){
                        ...
                        // Inform that the group is cancelled
                        Tuple t = Tuple("CANCELLED_SUSCRIPTION", "UNKNOWN", nickNameMultiplayer);
    ...
    if (modeMultiplayer != 0){
        Text complainText;
        if (numberPlayersGroup == 4){
            complainText.setString("GROUP IS COMPLETED, WAIT");
        }
        else {
            complainText.setString("OWNER CLOSED THE GROUP, WAIT");
        }
        complainText.setCharacterSize(static_cast<unsigned int>(int(45.0f * c.screenScale)));
        complainText.setFont(c.fontTitleMultiplayerMenu);
        complainText.setStyle(Text::Bold);
...
State Game::selectionModeMultiplayer(Configuration& c, SoundPlayer& r){
    ...
    if (modeMultiplayer == 0){
        ...
        // Control if the group is only formed by the owner
        ...
        if (escapePressed){
            ...
            // Display message to cancel the group
            Text complainText;
            if (emptyGroup){
                complainText.setString("NO GUESTS IN GROUP, WAIT");
            }
            else {
                complainText.setString("CANCELING THE GROUP, WAIT");
            }
            complainText.setCharacterSize(static_cast<unsigned int>(int(50.0f * c.screenScale)));
            complainText.setFont(c.fontTitleMultiplayerMenu);
            complainText.setStyle(Text::Bold);
        ...
        // Control if the guest has pressed escape key
        bool escapePressed = false;
        ...
        if (fail){
            ...
            if (escapePressed){
                ...
                complainText.setString("GROUP CANCELED BY OWNER, WAIT");
            }
            complainText.setCharacterSize(static_cast<unsigned int>(int(45.0f * c.screenScale)));
            complainText.setFont(c.fontTitleMultiplayerMenu);
            complainText.setStyle(Text::Bold);
    ...
    if (modeMultiplayer == 0){
        ...
        Text complainText;
        complainText.setString("GAME MODE SELECTED, WAIT");
        complainText.setCharacterSize(static_cast<unsigned int>(int(45.0f * c.screenScale)));
        complainText.setFont(c.fontTitleMultiplayerMenu);
        complainText.setStyle(Text::Bold);
        complainText.setFillColor(c.colorTitleTextMultiplayerMenu);
        ...
        Text complainText;
        complainText.setString("GAME MODE SELECTED BY OWNER, WAIT");
        complainText.setCharacterSize(static_cast<unsigned int>(int(50.0f * c.screenScale)));
        complainText.setFont(c.fontTitleMultiplayerMenu);
        complainText.setStyle(Text::Bold);
        complainText.setFillColor(c.colorTitleTextMultiplayerMenu);
...
State Game::selectionCircuitMultiplayer(Configuration& c, SoundPlayer& r){
    if (modeMultiplayer == 0){
        ...
        // Control if the guest pressed escape key
        bool escapePressed = false;
        ...
        if (fail){
            ...
            if (escapePressed){
                ...
                complainText.setString("CANCELED CIRCUIT BY OWNER, WAIT");
            }
            complainText.setCharacterSize(static_cast<unsigned int>(int(45.0f * c.screenScale)));
            complainText.setFont(c.fontTitleMultiplayerMenu);
            complainText.setStyle(Text::Bold);
        ...
        if (cancelledCircuit){
            ...
            Text complainText;
            complainText.setString("CIRCUIT CANCELED BY OWNER, WAIT");
            complainText.setCharacterSize(static_cast<unsigned int>(int(45.0f * c.screenScale)));
            complainText.setFont(c.fontTitleMultiplayerMenu);
            complainText.setStyle(Text::Bold);
            complainText.setFillColor(c.colorTitleTextMultiplayerMenu);
        ...
        Text complainText;
        complainText.setString("CIRCUIT SELECTED BY OWNER, WAIT");
        complainText.setCharacterSize(static_cast<unsigned int>(int(45.0f * c.screenScale)));
        complainText.setFont(c.fontTitleMultiplayerMenu);
        complainText.setStyle(Text::Bold);
        complainText.setFillColor(c.colorTitleTextMultiplayerMenu);
                    ...
                    for (xml_node<> *buttonNode = buttonProp->first_node(); buttonNode; buttonNode = buttonNode->next_sibling()){
                        ...
                        for (xml_node<> *button = buttonNode->first_node(); button; button = button->next_sibling()){
                            ...
                            // Get the colors of the button
                                ...
                                // Creation of the button and addition to the vector
                                posX = c.w.getSize().x / 2.f - 110.0f * c.screenScale;
                                posY = c.w.getSize().y / 2.f - (-88.0f - idButton * 83.f) * c.screenScale;
                                widthButton = 200.0f * c.screenScale;
                                heightButton = 50.0f * c.screenScale;
...
State Game::makeConnectionServerTest(Configuration& c, SoundPlayer& r){
    ...
    if (!c.multiplayerTestConnectionMenuRead){
        ...
        // Change the state of the first color
        Button b = Button(c.w.getSize().x / 2.f - 110.0f * c.screenScale, c.w.getSize().y / 2.f + 88.0f * c.screenScale,
        ...
        for (int i = 1; i < numButtons; i++){
            // Change the state of the first color
            Button b = Button(c.w.getSize().x / 2.f - 110.0f * c.screenScale, c.w.getSize().y / 2.f - (-88.0f - i * 83.f) * c.screenScale,
                              200.0f * c.screenScale, 50.0f * c.screenScale, c.fontMenuMultiplayerTestConnectionButtons,
    ...
    minLatencyValue.setPosition((c.w.getSize().x / 2.f) + 90.f * c.screenScale,
                                c.w.getSize().y / 2.f - 150.f * c.screenScale);
    ...
    if (latency <= 40){
        ...
        indicator = "YOUR CLIENT WILL GO DECENTLY";
    }
    else if (latency <= 100){
        indicator = "YOUR CLIENT WILL GO A LITTLE LAGGED";
    }
    else {
        indicator = "YOUR CLIENT WILL GO VERY LAGGED";
    }
...
void Game::storingMultiplayerCars(){
    ...
    // Initialize the position of the player
    posArrival = codePlayerInGroup;
    string pathFileMultiplayerCar = "";
    int typeVehicle, colorVehicle, numTextures;

    // Store the rival cars in the vector
    for (int i = 1; i <= numberRacers; i++) {

        if (i != codePlayerInGroup){
            // Set the position of the car in the axis X
            if (i % 2 != 0){
                positX = -0.3f;
            }
            else {
            ...
            if (firstRace){
                typeVehicle = multiplayerCarsRanking[i - 1].getVehicle();
                colorVehicle = multiplayerCarsRanking[i - 1].getColor();
            }
            else {
                typeVehicle = groupDataPlayers[i - 1].getVehicle();
                colorVehicle = groupDataPlayers[i - 1].getColor();
            }
            ...
            switch(typeVehicle){
                case 0:
                    vehicle = "Data/Vehicles/Motorbikes/Motorbike" + to_string(colorVehicle + 1);
                    scaling = 1.15f;
                    numTextures = 88;
                    break;
                case 1:
                    vehicle = "Data/Vehicles/Devastators/Devastator" + to_string(colorVehicle + 1);
                    scaling = 1.85f;
                    numTextures = 85;
                    break;
                case 2:
                    vehicle = "Data/Vehicles/Minivans/Minivan" + to_string(colorVehicle + 1);
                    scaling = 1.75f;
                    numTextures = 77;
                    break;
                case 3:
                    vehicle = "Data/Vehicles/Trucks/Truck" + to_string(colorVehicle + 1);
                    scaling = 2.02f;
                    numTextures = 103;
                    break;
                case 4:
                    vehicle = "Data/Vehicles/FormulaOnes/FormulaOne" + to_string(colorVehicle + 1);
                    scaling = 1.80f;
                    numTextures = 86;
                    break;
                case 5:
                    vehicle = "Data/Vehicles/Police";
                    scaling = 1.85f;
                    numTextures = 74;
            }
...
bool Game::getOnMultiplayerMode(){
