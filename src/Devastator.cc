...
void Devastator::hitControl(const bool vehicleCrash) {
    ...
    if (acceleration <= minCrashAcc ||
        current_code_image == 44 || current_code_image == 52)
    {
...
Vehicle::Direction Devastator::rotationControl(Configuration &c, float curveCoefficient) {
    skidding = false;
...
void Devastator::drawInitialAnimation(Configuration &c, float x, bool &end) {
    if (textures.size() == Devastator_vehicle::PLAYER_TEXTURES) {
        if (counter_code_image >= maxCounterToChange) {
            current_code_image++;
            counter_code_image = 0;
...
void Devastator::drawGoalAnimation(Configuration &c, int &step, bool &end, bool smoke) {
    if (textures.size() == Devastator_vehicle::PLAYER_TEXTURES) {
        if (counter_code_image >= maxCounterToChange) {
            current_code_image++;
            counter_code_image = 0;
