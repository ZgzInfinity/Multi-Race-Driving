...
Vehicle::Action Vehicle::accelerationControl(Configuration* c, bool hasGotOut) {
    Action a = NONE;

    if (Keyboard::isKeyPressed(c->getBrakeKey()))
        a = BRAKE;

    if (a != BRAKE && Keyboard::isKeyPressed(c->getAccelerateKey())) {
        if (hasGotOut) {
...
Vehicle::Direction Vehicle::rotationControl(Configuration* c, float curveCoefficient) {
    if (speed > 0.0f) {
...
void Vehicle::draw(Configuration* c, const Action &a, const Direction &d, const Map::Elevation &e, RenderWindow* app) {
    if (a != NONE) {
    ...
    minScreenX = ((float)app->getSize().x) / 2.0f - sprite.getGlobalBounds().width / 2.0f;
    maxScreenX = minScreenX + sprite.getGlobalBounds().width;
    sprite.setPosition(minScreenX, ((float)app->getSize().y) * 0.84 - sprite.getGlobalBounds().height / 2.0f);
    app->draw(sprite);
}
